...
public class StoreService {
    ...
    public List<StoreSearchResponse> searchStoresByName(final String storeName, final Pageable pageable) {
        final List<Store> stores = storeRepository.findStoresByStoreNameContaining(storeName, pageable);

        return stores.stream()
                .map(store -> StoreSearchResponse.of(store, postRepository.countByStore(store)))
                .toList();
    }
}
